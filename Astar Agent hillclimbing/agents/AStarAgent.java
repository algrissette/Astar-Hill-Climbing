...
public class AStarAgent extends Agent {
  ...
  */
    ...
    {
    	//we take into account that a new obstacle has been created so we add that when creating our path again
    		Unit.UnitView enemyFootmanUnit = state.getUnit(enemyFootmanID);
    		MapLocation enemyFootmanLoc = new MapLocation(enemyFootmanUnit.getXPosition(),enemyFootmanUnit.getYPosition(), null);
   ...
   //Creating a priority queue of maplocations 
    	//using comparator but in order to do this we need to set what needs to be compared and how
    	//we compare the .f values because these are our heurisic + g (important for A*)
    	//now when we add something to heap, if it has a less h then it will be at the top
    	
    	
    	PriorityQueue<MapLocation> heap = new PriorityQueue<MapLocation>(
    	...
    	/*we make a hashtable of nodes we have finalized for their shortest distance for liner 
    	runtime data retrieval */
    	 
    	Set<MapLocation> finalizedVerticies = new HashSet<MapLocation>();
    	Map<MapLocation, Float> path2Cost = new HashMap<MapLocation, Float>();
    	//we add start to the heap because the path to that is itself
    	//whenever we add something to the heap we add the cost of the path into a hashtable
    	//goal scenario: the goal path is sitting at the top of the heap and we can pull it to trace it back
    
    	
    	heap.add(start);
    	path2Cost.put(start, start.f);
    	while(!heap.isEmpty()) {
    		MapLocation currentMinPath=heap.poll();
    		finalizedVerticies.add(currentMinPath);
    		
    		if (currentMinPath.equals(goal)) {
    			return this.tracePath(start,currentMinPath.cameFrom , new Stack<MapLocation>());
    		}
    		for (MapLocation childPath : this.getAndCheckNeighbors(currentMinPath.g+ 1, currentMinPath, goal, xExtent,
    													 yExtent, enemyFootmanLoc, resourceLocations)) {
    			if(!finalizedVerticies.contains(childPath)) {
    				
    				//childpath could be brand new
    				if (!path2Cost.containsKey(childPath)){
    					heap.add(childPath);
    					path2Cost.put(childPath, childPath.f);
    				}
    				else {
    					Float oldCost= path2Cost.get(childPath);
    					if (oldCost> childPath.f) {
    						heap.remove(childPath);
    						heap.add(childPath);
    						path2Cost.replace(childPath, childPath.f);
    					}
    					
    				}
    				//or already seen before but not popped
